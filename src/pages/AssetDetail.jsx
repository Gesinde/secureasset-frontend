...
import { getAssetById, assignCustodian, acceptCustody, regenerateQR } from '../services/assetService';
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="p-8 max-w-3xl mx-auto">
        <Link to="/assets" className="text-blue-600 dark:text-blue-400 hover:underline text-sm">
          ← Back to Assets
        </Link>
        {canEdit && (
          <Link to={`/assets/${asset._id}/edit`} className="text-blue-600 dark:text-blue-400 hover:underline text-sm ml-4">
            Edit Asset
          </Link>
        )}

        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 mt-4 flex flex-col md:flex-row gap-8">
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{asset.name}</h1>
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                ...
                <dd className="text-gray-900 dark:text-white">{asset.location}</dd>
              </div>
              <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                <dt className="text-gray-500 dark:text-gray-400">Status</dt>
                <dd className="text-gray-900 dark:text-white">{asset.status.replace('_', ' ')}</dd>
              </div>
              <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                <dt className="text-gray-500 dark:text-gray-400">Created</dt>
                <dd className="text-gray-900 dark:text-white">{new Date(asset.createdAt).toLocaleDateString()}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500 dark:text-gray-400">Last Verified</dt>
                <dd className={asset.lastVerifiedAt && daysSince(asset.lastVerifiedAt) > 90 ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-900 dark:text-white'}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              <p className="text-gray-500 dark:text-gray-400 text-sm mb-2">Custodian</p>
              {asset.custodian ? (
                <div>
                  <p className="text-gray-900 dark:text-white text-sm">{asset.custodian.name} ({asset.custodian.role})</p>
                ...
                <p className="text-gray-500 dark:text-gray-500 text-sm">No custodian assigned</p>
              )}
                  ...
                  {showAssignForm ? (
                    <div className="flex gap-2">
                      <select
                        ...
                        className="flex-1 px-2 py-1.5 rounded bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-xs border border-gray-300 dark:border-gray-600"
                      >
                      ...
                      <button onClick={() => setShowAssignForm(false)} className="text-gray-500 dark:text-gray-400 text-xs">
                        Cancel
                      </button>
                    </div>
                  ) : (
