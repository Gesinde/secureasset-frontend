...
function Transfers() {
  ...
  const fetchData = async () => {
    try {
      const [transferData, assetData] = await Promise.all([
        getTransfers(),
        getAssets(),
      ]);
  ...
  useEffect(() => {
    ...
    getUsers({ department: formData.toDepartment })
      .then(setRecipients)
      .catch(() => setRecipients([]));
  ...
  const canActOn = (transfer) => {
    ...
    if (transfer.status === 'pending_sender_hod') {
      return (
        ...
        user.department === transfer.fromDepartment
      );
    }
    ...
    if (transfer.status === 'pending_recipient_hod') {
      return (
        user.role === 'department_head' &&
        user.department === transfer.toDepartment
      );
    }
      ...
      <div className="p-8">
        ...
        {showForm && (
          <form
            ...
            className="bg-white dark:bg-gray-800 p-6 rounded-lg mb-6 space-y-4"
          >
            ...
            <div>
              ...
              <select
                ...
                required
                className="w-full px-3 py-2 rounded bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 focus:outline-none focus:border-blue-500"
              >
                <option value="">
                  Select asset
                </option>
                ...
                {assets.map((a) => (
                  <option key={a._id} value={a._id}>
                    {a.name} ({a.serialNumber}) — {a.department}
                  </option>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 focus:outline-none focus:border-blue-500 disabled:opacity-50"
              >
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-white font-semibold">
                      {t.asset?.name} ({t.asset?.serialNumber})
                    </p>
                  ...
                  <span
                    className={`px-2 py-1 rounded text-xs whitespace-nowrap ${STAGE_COLOR[t.status]}`}
                  >
                    ...
                    {rejectingId === t._id ? (
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <input
                          ...
                          className="flex-1 px-3 py-1.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white text-xs border border-gray-300 dark:border-gray-600"
                        />
